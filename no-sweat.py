...
def preprocess(df_, benifits_type):
    ...
    match benifits_type:
        case "老人企业补贴":
            df["企业补贴"] = (
                df["补贴更正"] + df["误餐补贴"] + df["补发补贴"] - df["扣款_补贴"]
            )
    ...
    if not df_.query("提高待遇>0").empty:
        result = pd.concat(
            [
                ...
                df_.loc[df_["提高待遇"] > 0, ["x_银行帐号", "姓名", "提高待遇"]]
                .copy()
                .rename(columns={"提高待遇": "企业补贴"}),
            ]
        )
    ...
    if not df_.query("提高待遇>0").empty:
        result = pd.concat(
            [
                df_.loc[
                    df_["企业补贴"] > 0,
                    ["企业补贴", "姓名", "x_银行帐号", "开户行", "行号"],
                ].copy(),
                df_.loc[
                    df_["提高待遇"] > 0,
                    ["提高待遇", "姓名", "x_银行帐号", "开户行", "行号"],
                ]
                .copy()
                .rename(columns={"提高待遇": "企业补贴"}),
            ]
        ...
        result = df_.loc[
            df_["企业补贴"] > 0, ["企业补贴", "姓名", "x_银行帐号", "开户行", "行号"]
        ].copy()
    if result.empty:
        return None
    return result.values.tolist()
...
def group_by(df, key, agg_dict):
    data = df.query("re == 1").groupby(key).agg(agg_dict).reset_index()
    return data
...
def make_bank_count(source, term, bank):
    pass


def make_bank_report(df, term, bank, out_base):
    if bank == "工商银行":
        df.loc[
            df["发放银行"].isin(
                [
    ...
    print(df)
    title_name = f"{term[:4]+"年" + str(int(term[-2:]))+"月河南油田退休人员企业补贴"+bank+"代发汇总表"}"
    file_name = out_base / f"{bank+"拨付汇总表-"+ term + ".xlsx"}"

    arr = df.filter(items=["项目", "x_银行帐号", "实发补贴", "备注"]).values.tolist()

    report_bank(arr, title_name, file_name)


def report_bank(arr, title_name, output):
    arr_len = len(arr)
    app = xw.App(visible=False)
    wb = app.books.add()
    sht = wb.sheets[0]

    title = sht.range("A1:E2")
    title.merge()
    title.value = title_name
    title.api.font.name = "宋体"
    title.api.font.bold = True
    title.api.font.size = 20

    signature = sht["A3"]
    signature.value = "单位：河南油田社保中心养老保险室"
    ...
    sht[f"A{arr_len+5}:B{arr_len+5}"].merge()
    sht[f"A{arr_len+5}:B{arr_len+5}"].value = "合计"
    sht[f"A{arr_len+5}"].api.HorizontalAlignment = -4108
    sht[f"A{arr_len+5}"].api.font.bold = True

    total = sht[f"D{arr_len+5}"]
    total.api.font.bold = True
    total.formula = f"=SUM(D5:D{arr_len+4})"

    footer = sht[f"A{arr_len+7}"]
    footer.value = "制表：曾理"
    footer.api.font.name = "仿宋"
    footer.api.font.size = 14

    footer = sht[f"E{arr_len+7}"]
    footer.value = "打印日期：" + str(date.today())
    footer.api.font.name = "仿宋"
    footer.api.font.size = 14

    # 水平居中
    sht["A1"].api.HorizontalAlignment = -4108
    sht[f"A4:E{arr_len+5}"].api.HorizontalAlignment = -4108

    # 设置列宽
    sht[f"A3:A{arr_len+4}"].column_width = 17
    sht[f"B3:B{arr_len+4}"].column_width = 45
    sht[f"C3:C{arr_len+4}"].column_width = 10
    sht[f"D3:D{arr_len+4}"].column_width = 27
    sht[f"E3:E{arr_len+4}"].column_width = 34

    # 设置行高
    sht["A1"].row_height = 56
    sht["A3"].row_height = 32
    sht[f"A4:E{arr_len+5}"].row_height = 25

    # 设置边框
    content = sht[f"A4:E{arr_len+5}"]
    ...
    # 纵向打印
    # sht.api.PageSetup.Orientation = xw.constants.PageOrientation.xlPortrait
...
def make_change_report(df, term, type, output_base):
    ...
    match type:
        case "老人":
            ...
            file_name = (
                output_base / f"{"离退休职工（老人）人员变动汇总表-"+ term + ".xlsx"}"
            )
        case "集体工":
            info = (
                df.query(
...
def report_change(arr, title_name, output):
    ...
    # 横向打印
    sht.api.PageSetup.Orientation = 2  # 1为纵向，2为横向
    # 纵向打印
...
if __name__ == "__main__":
    ...
    export_data(
        ...
        / f"{"老人企业补贴(工行报盘)-"+current_term+".xlsx"}",
    )
    export_data(
        conv_cbc(preprocess(df_lr, "老人企业补贴")),
        bank_templates["建行"],
        dest_path(dest_base, current_term)["建行"]
        / f"{"老人企业补贴(建行报盘)-"+current_term+".xls"}",
    )
    ...
    export_data(
        conv_bocny(preprocess(df_lr, "老人企业补贴")),
        bank_templates["中行-南阳"],
        dest_path(dest_base, current_term)["中行"]
        / f"{"老人企业补贴(中行-南阳报盘)-"+current_term+".xlsx"}",
    )

    export_data(
        conv_icbc(preprocess(df_jtg, "集体工企业补贴")),
        bank_templates["工行"],
        dest_path(dest_base, current_term)["工行"]
        / f"{"集体工企业补贴(工行报盘)-"+current_term+".xlsx"}",
    )
    export_data(
        conv_cbc(preprocess(df_jtg, "集体工企业补贴")),
        bank_templates["建行"],
        dest_path(dest_base, current_term)["建行"]
        / f"{"集体工企业补贴(建行报盘)-"+current_term+".xls"}",
    )
    export_data(
        conv_bocyt(preprocess(df_jtg, "集体工企业补贴")),
        bank_templates["中行-油区"],
        dest_path(dest_base, current_term)["中行"]
        / f"{"集体工企业补贴(中行-油区报盘)-"+current_term+".xlsx"}",
    )
    export_data(
        conv_bocny(preprocess(df_jtg, "集体工企业补贴")),
        bank_templates["中行-南阳"],
        dest_path(dest_base, current_term)["中行"]
    ...
    out_lr = (
        ...
        / f"{current_term[:4]+"年"}"
        / f"{str(int(current_term[-2:]))+"月"}"
        / f"{"老人企业补贴汇总-"+ current_term+r".xlsx"}"
    )
    out_jtg = (
        Path(dest_base)
        / f"{current_term[:4]+"年"}"
